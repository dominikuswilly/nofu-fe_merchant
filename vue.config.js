  ...
  chainWebpack: config => {
    config
      ...
      .tap(args => {
        ...
        const backendUrl = process.env.VUE_APP_BACKEND_URL || 'http://apinofudev.bengkelfajarjaya.com/api/customer'
  ...
  devServer: {
    ...
    proxy: {
      '/api/customer': {  // Proxy any /api/* calls
        target: 'http://apinofudev.bengkelfajarjaya.com/api/customer',  // Your backend URL
